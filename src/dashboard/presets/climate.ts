...
import { blockId, ofDomain, isLive, isNumericSensor, leafName } from './index';
...
function trendSeries(states: PresetContext['states'], climates: HassEntity[]): ChartSeries[] {
  const series: ChartSeries[] = [];
  // Prefer dedicated temperature sensors; fall back to the climates themselves.
  const tempSensors = Object.values(states)
    .filter((e) => isNumericSensor(e) && e.attributes.device_class === 'temperature')
    .slice(0, 4);
...
export function buildClimate(ctx: PresetContext): Surface {
  const { states } = ctx;
  const climates = ofDomain(states, 'climate').filter(isLive);
  const humidifiers = ofDomain(states, 'humidifier').filter(isLive);
  ...
  const series = trendSeries(states, climates);
  if (series.length) {
